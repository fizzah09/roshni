...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="bg-surface-dim border-b border-outline-variant sticky top-0 w-full z-50">
      <div className="flex justify-between items-center w-full px-4 md:px-8 py-3 max-w-7xl mx-auto">
        <div
          className="flex items-center gap-2 cursor-pointer group"
          onClick={() => onSelectView('home')}
        >
          <div className="p-1.5 rounded bg-primary/10 border border-primary/30 group-hover:scale-105 transition-transform">
            <Bolt className="w-6 h-6 text-primary fill-primary" />
          </div>
          <span className="text-xl md:text-2xl font-bold tracking-tighter text-primary font-mono">
            {lang === 'en' ? 'ROSHNI_SYSTEM' : 'روشنی سسٹم'}
          </span>
        </div>

        <nav className="hidden md:flex gap-4 items-center text-sm font-mono font-medium">
          {[
