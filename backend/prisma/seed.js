...
async function main() {
  console.log('Seeding Standalone Express ROSHNI database with official DISCOs & KE 4-cycle schedules...');
  ...
  const gepco = await prisma.disco.create({
    data: { code: 'GEPCO', name: 'Gujranwala Electric Power Company', region: 'Gujranwala & Sialkot' }
  });

  const fesco = await prisma.disco.create({
    data: { code: 'FESCO', name: 'Faisalabad Electric Supply Company', region: 'Faisalabad & Sargodha' }
  });
  ...
  console.log('Standalone Express ROSHNI Database Seeded Successfully!');
}
